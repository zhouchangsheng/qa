  ...
  $.login = function(text, title, onOK, onCancel, username, password) {
    ...
    var modal = $.modal({
      text: '<p class="weui-prompt-text">'+(config.text || '')+'</p>' +
            ...
            '<input type="userPassword" class="weui_input weui-prompt-input" id="weui-prompt-userPassword" value="' + (config.userPassword || '') + '" placeholder="输入密码" />',
      title: config.title,
      autoClose: false,
      buttons: [
      {
        ...
        onClick: function() {
          ...
          var password = $("#weui-prompt-userPassword").val();
          if (!config.empty && (username === "" || username === null)) {
            modal.find('#weui-prompt-username').focus()[0].select();
            return false;
          }
          if (!config.empty && (password === "" || password === null)) {
            modal.find('#weui-prompt-userPassword').focus()[0].select();
            return false;
          }
          $.closeModal();
